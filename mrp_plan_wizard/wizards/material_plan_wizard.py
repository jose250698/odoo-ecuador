...
class MaterialPlanWizardLine(models.TransientModel):
    ...
    product_qty = fields.Float(string='Planned Quantity', digits_compute=dp.get_precision(
        'Product Unit of Measure'), required=True)
    product_uom_id = fields.Many2one('product.uom', 'UoM', required=True)
    qty_available = fields.Float(string='Available', digits_compute=dp.get_precision(
        'Product Unit of Measure'), compute='_get_product_info', readonly=True)
    qty_forecasted = fields.Float(string='Forecasted', digits_compute=dp.get_precision(
        'Product Unit of Measure'), compute='_get_product_info', readonly=True)
    stock_uom_id = fields.Many2one('product.uom', string='UoM', compute='_get_product_info')
    qty_needed = fields.Float(string='Minimum need', digits_compute=dp.get_precision(
        'Product Unit of Measure'), compute='_get_product_info', default=0, readonly=True)
    ...
    def _get_product_info(self):
        ...
        for line in self:
            product = line.product_id.with_context(warehouse=self.wizard_id.warehouse_id.id)
            line.qty_available = product.qty_available_not_res
            line.qty_forecasted = product.virtual_available
...
class MaterialPlanWizard(models.TransientModel):
    ...
    warehouse_id = fields.Many2one('stock.warehouse', string='Warehousé', reuired=True)
    ...
    def _action_calculate_ingredients(self):
        ...
        for line in self.planned_items:
            ...
            _logger.debug(u"%s: %s %s (Lager: %s %s)", line.product_id.name, line.product_qty,
                          line.product_uom_id.name, line.product_id.qty_available, line.product_uom_id.name)
        ...
        for product, qty in purchase_list.iteritems():
            ...
            if diff < 0:
                diff = 0
            _logger.debug(u"%s: %s %s (Lager: %s %s | Einkaufsbedarf: %s %s)", product.name, float_round(
                qty, precision_digits=digits), product.uom_id.name, product.qty_available, product.uom_id.name, diff, product.uom_id.name)
            record_list.append(
                (0, 0, {
