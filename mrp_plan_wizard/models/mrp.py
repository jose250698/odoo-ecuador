    ...
    def _get_ingredients_recursive(self, ilist, properties=None, ingredients=None, production_list=None):
        ...
        for line in ilist:
            ...
            qty = self.env['product.uom']._compute_qty(
                line['product_uom'], line['product_qty'], product.uom_id.id)
            if bom:
                ...
                if len(bom.ids) > 1:
                    if product in ingredients:
                        ...
                        ingredients[product] = qty
                    _logger.debug(
                        "More than one Bill of Material found, so we do not care about it by now and count it as a needed ingredient!")
                else:
                    if bom.type == 'normal':
